import { ChangeDetectionStrategy, ChangeDetectorRef, Component, Inject, Input, OnInit, Optional, Self, ViewChild } from '@angular/core';
...
export class DatePickerComponent extends ControlComponent<Date | null> implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.ngControl) {
      ...
      // FIXME Will not work nicely in all cases, because `control` works with `Date`, and `embeddedControl` works with `Moment` (usually)
